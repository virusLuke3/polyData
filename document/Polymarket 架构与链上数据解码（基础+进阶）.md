...
结合当前仓库实现，sync_state 实际已经扩展为多条同步链分别维护 checkpoint：
- `market_sync`：常规 market 增量同步断点
- `market_sync_live`：live 模式 market 断点
- `trade_sync`：历史 trades 回填断点
- `trade_sync_live`：live 模式 trades 断点
- `oracle_sync`：历史 oracle 回填断点
- `oracle_sync_live`：live 模式 oracle 断点

另外，当前工程默认数据库后端已经切到 MySQL。很多脚本仍保留 `--db /path/to/polymarket_indexer.db` 参数用于兼容旧接口，但在默认 MySQL 模式下，真实写入目标由 `POLYMARKET_DB_BACKEND=mysql` 和 `POLYMARKET_MYSQL_*` 环境变量决定，而不是该 SQLite 文件路径本身。
## 任务拆解
